...
const Marquee = () => {
  const svgSeparator = (
    <svg
      ...
      viewBox="0 0 20 22"
      fill="none"
      className="inline-block mx-2 text-[24px] " // Add some margin around the SVG
    >
      <path
  ...
  const text = "Free express shipping worldwide";
  const repetitions = 10;

  return (
    <div className="relative overflow-hidden bg-[rgb(153,21,242)] py-3 h-[92px] flex items-center text-white">
      <div className="group">
        <div className="marquee-content whitespace-nowrap group-hover:[animation-play-state:paused] animate-scroll">
          {[...Array(repetitions)].map((_, i) => (
            <React.Fragment key={i}>
              <strong className="text-[24px] mr-[15px] ml-[15px]">
                {text}
              </strong>
              {i < repetitions - 1 && svgSeparator}
            </React.Fragment>
          ))}
        </div>
      </div>
